...
public class LiveDataScheduler {
    ...
    public void pollLiveFixtures() {
        //log.info("==================== 1분 단위 실시간 데이터 스케줄링 시작 ================================");
        ...
        //log.info("1분 단위 스케줄링 걸린 시간={}", duration);
        //log.info("==================== 1분 단위 실시간 데이터 스케줄링 종료 ================================");
    }
    ...
    private LiveEventDto fetchLiveData(Round currentRound) {
        return fpl.get()
                .uri("/event/{round}/live/", currentRound.getRound())
                .retrieve()
                .bodyToMono(LiveEventDto.class)
                .doOnError(e -> log.error("Live API 호출 실패 - 라운드: {}", currentRound.getRound(), e))
                .blockOptional()
                .orElseThrow(() -> new IllegalStateException("Live 데이터가 비어있습니다"));
    }
